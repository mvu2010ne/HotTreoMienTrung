...
def load_or_get_cookie():
    ...
    cookie = input(gradient_tutu("""\n╭─────[ Nhập Cookie Facebook ] - [ Kazuto | Minh Vũ ]
│ 
╰─➤   """)).strip()
    if check_cookie(cookie):
...
def spam_all_groups(cookie, delay):
    ...
    print(f"\033[1;33m[ Kazuto ] \033[1;32mBot Đang Được Triển Khai Với Số Lượng Là \033[1;33m{len(group_ids)} \033[1;32mNhóm (delay \033[1;33m{delay}s\033[1;32m)")
    while True:
...
def spam_treo_nhap(cookie, delay, thread_id):
    message = input("Nhập nội dung tin nhắn muốn spam: ").strip()
    if not message:
        print("\033[1;31m❌ Không được để trống nội dung!")
        return
...
def spam_treo_ngon(cookie, delay, thread_id, message_file):
    try:
        with open(message_file, "r", encoding="utf-8") as f:
            full_message = f.read().strip()  # Đọc toàn bộ nội dung file
            if not full_message:
                print("\033[1;31m❌ File không có nội dung!")
                return
    except:
    ...
    while True:
        message = full_message  # Gửi toàn bộ nội dung
        ok = send_message(cookie, user_id, thread_id, message)
        print(f"\033[1;36mMessager: \033[1;33m{message}\n\033[1;36mThread ID: \033[1;33m{thread_id}\n")
        index += 1
        time.sleep(delay)
...
def spam_lag_in_code(cookie, delay, thread_id):
    ...
    while True:
        ...
        ok = send_message(cookie, user_id, thread_id, message)
        print(f"\033[1;33m[ Kazuto ] \033[1;32mĐã Gửi Tin Nhắn Unicode Lag Đến Nhóm \033[1;33m{thread_id}")
        index += 1
        time.sleep(delay)
...
def spam_unicode_invisible(cookie, delay, thread_id):
    ...
    while True:
        ...
        print(f"\033[1;33m[ Kazuto ] \033[1;32mĐã Gửi Thành Công Unicode Ẩn Lần Thứ \033[1;33m{count+1}")
        count += 1
        time.sleep(delay)
        
def spam_treo_file_kem_ten(cookie, delay, thread_id, ten_nguoi_nhan, message_file, vi_tri_ten="s"):
    try:
        with open(message_file, "r", encoding="utf-8") as f:
    ...
    while True:
        for line in lines:
            if vi_tri_ten.lower() == "t":
                message = f"{ten_nguoi_nhan} {line}"  # Tên ở đầu
            else:
                message = f"{line} {ten_nguoi_nhan}"  # Tên ở cuối (mặc định)
...
def menu():
    ...
    while True:
        print(gradient("🧑‍💻 Admin: Minh Vũ w Meo Cte"))
        print(gradient("📱 Zalo: https://zalo.me/g/hmkwri283"))
        print(gradient("📢 Nhóm Telegram: https://t.me/dechecuakazuto"))
        print(gradient("📘 Nhóm Zalo 2 : https://zalo.me/g/fguxml824"))
        print(gradient("💗 Nhóm Zalo 3 : https://zalo.me/g/ljpvuh636"))
        print(gradient("💬 Zalo : https://zaloapp.com/qr/p/c3lut86z7wiy?src=qr"))
        print(gradient("👥 Nhóm Messenger: https://www.messenger.com/t/23968960736140095"))
        print("    \033[1;31m ╔═                                        ═╗")
        print(gradient("       ┌──────────────────────────────────────┐"))
        print(gradient("       │   Credits : Kazuto | Minh Vũ   │"))
        print(gradient("       └──────────────────────────────────────┘"))
        print("    \033[1;31m ╚═                                        ═╝")
        print(gradient("\n╔════════════════════════════════════════════╗"))
        print(gradient("║       🤖 MENU BOT MESSENGER BY Kazuto      ║"))
        print(gradient("╠════════════════════════════════════════════╣"))
        print(gradient("║ 0. Rải tin nhắn nhiều nhóm                 ║"))
        print(gradient("║ 1. Treo ngôn 1 nhóm duy nhất               ║"))
        print(gradient("║ 2. Treo ngôn bằng nội dung                 ║"))
        print(gradient("║ 3. Gửi spam lag (code)                     ║"))
        print(gradient("║ 4. Treo Có Tên.                            ║"))
        print(gradient("║ 5. Gửi Unicode tàng hình                   ║"))
        print(gradient("║ 6. Thoát.                                  ║"))
        print(gradient("╚════════════════════════════════════════════╝"))

        choice = input(gradient_tutu("""╭─────[ Nhập Lựa Chọn ] - [ Kazuto | Minh Vũ ]
│ 
╰─➤  """))
...
│ 
╰─➤  """))
            message_text = input_with_box(gradient_tutu("""\n╭─────[ Nhập Nội Dung Muốn Spam ] - [ Kazuto | Minh Vũ  ]
│ 
╰─➤   """))
            spam_treo_nhap(cookie, delay, thread_id)
...
│ 
╰─➤  """))
           ten_nguoi_nhan = input_with_box(gradient_tutu("""\n╭─────[ Nhập Tên Người Nhận ] - [ Kazuto | Minh Vũ ]
│ 
╰─➤  """))
           vi_tri_ten = input(gradient_tutu("""\n╭─────[ Thêm Tên ở Trước (t) hay Sau (s)? ] - [ Kazuto | Minh Vũ ]
│ 
╰─➤  """)).strip().lower()
           spam_treo_file_kem_ten(cookie, delay, thread_id, ten_nguoi_nhan, message_file, vi_tri_ten)
...
def spam_all_groups(cookie, delay):
    ...
    print(f"\033[1;33m[ Kazuto ] \033[1;32mBot Đang Triển Khai Gửi Tin Nhắn Vào \033[1;33m{len(group_ids)} \033[1;32mNhóm (delay\033[1;33m {delay}s\033[1;32m)")
    while True:
        for thread_id in group_ids:
            message = random.choice(messages)
            ok = send_message(cookie, user_id, thread_id, message)
